...
var klaw_sync = __nccwpck_require__(9036);
...
async function findResources(path) {
  const ids = [];
  try {
    const files = (0,external_fs_.readdirSync)(path);

    for (const file of files) {
      const xmlData = (0,external_fs_.readFileSync)(path + '/' + file, { encoding: 'utf8', flag: 'r' });
...
function internalFindAllValuesDirectories(pathToProject) {
  ...
  return klaw_sync(pathToProject, {
    filter: klawFilter,
    depthLimit: -1,
    traverseAll: true,
    nofile: true
...
async function internalAction(config) {
  ...
  config.project = (0,external_path_.resolve)(config.project);

  if (config.verbose) {
    (0,core.info)('Project path: ' + config.project);
  }
  ...
  if (config.verbose) {
    (0,core.info)('Found directories: ' + foundDirectories);
  }
  ...
  for (const directory of foundDirectories) {
    ...
    if (config.verbose) {
      (0,core.info)('Processsing directory: ' + directory);
    }
    ...
    if (config.verbose) {
      (0,core.info)('Module: ' + JSON.stringify(moduleObj, null, 2));
    }
  ...
  if (config.verbose) {
    (0,core.info)('Total stats: ' + JSON.stringify(totalStats, null, 2));
  }

  (0,external_fs_.writeFileSync)(config.report, JSON.stringify(report, null, 2));
...
const projectPath = (0,core.getInput)('PROJECT_PATH');
const reportPath = (0,core.getInput)('REPORT_PATH');
const supportedLanguages = (0,core.getInput)('SUPPORTED_LANGUAGES');
const statsOnly = (0,core.getInput)('STATS_ONLY');
const verbose = (0,core.getInput)('VERBOSE');
...
(async () => {
  ...
  (0,core.setOutput)('result', result);
})().catch(err => {
    (0,core.error)(err);
